...
OUTPUT_FILE = 'ma_regime_labeled_data_with_perfect_v52_test.csv'
SCORING_FILE = 'regime_scoring_metrics_v52_test.csv'
TIMEFRAME = '15min'
...
ENHANCED_PARAMS = {
    ...
    'between_scale_sensitivity': 3.0,  # Near-zero BETWEEN in non-flat slopes
    'volatility_scale_factor': 1.0      # Strong volatility adjustment
}

PERFECT_PARAMS = {
    'min_forward_bars': 100,    # Reduced for efficiency
    'max_forward_bars': 200,    # Adaptive range
    'adaptive_forward': True,
    'perfect_lookahead': 100    # Balanced for speed and accuracy
}
...
def apply_regime_classification(data, use_perfect=False):
    ...
    if use_perfect:
        for i in tqdm(range(len(data)), desc=desc, ncols=80):
            if i >= CORE_PARAMS['long_ma_period'] and i + PERFECT_PARAMS['perfect_lookahead'] < len(data):
                # Adaptive lookahead based on volatility
                vol_ratio = data['Volatility_Ratio'].iloc[i]
                lookahead = PERFECT_PARAMS['min_forward_bars'] if vol_ratio > 1.2 else PERFECT_PARAMS['max_forward_bars']
                # Scan forward to find first stable regime
                for j in range(i, min(i + lookahead, len(data))):
...
def apply_transition_aware_persistence(data, regime_col, confirmed_col):
    ...
    for i in range(len(data)):
        ...
        if is_live_data:
            ...
            # No persistence for perfect system; trust lookahead
            data.iloc[i, data.columns.get_loc(confirmed_col)] = current_raw
