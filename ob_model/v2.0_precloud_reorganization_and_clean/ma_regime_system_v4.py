...
OUTPUT_FILE = 'ma_regime_labeled_data_with_perfect_v34_test.csv'
SCORING_FILE = 'regime_scoring_metrics_v34_test.csv'
TIMEFRAME = '15min'
...
def calculate_helper_columns(data):
    ...
    # Initialize perfect indicator columns
...
def apply_regime_classification(data, use_perfect=False):
    ...
    confirmed_col = 'Perfect_Confirmed_Regime' if use_perfect else 'Live_Confirmed_Regime'
    ...
    for i in tqdm(range(len(data)), desc=desc, ncols=80):
        if i >= CORE_PARAMS['long_ma_period'] and (not use_perfect or i + perfect_lookahead < len(data)):
            # For perfect system, calculate indicators for the future window
            if use_perfect:
                ...
                if i > 0:
                    prev_sma_13 = data.loc[data.index[i-1], 'Perfect_SMA_13']
                    if pd.notna(prev_sma_13) and prev_sma_13 != 0:
                        data.loc[data.index[i], 'Perfect_SMA_13_Slope'] = data.loc[data.index[i], 'Perfect_SMA_13'] / prev_sma_13 - 1
                ...
                perfect_slope = data.loc[data.index[i], 'Perfect_SMA_13_Slope']
                if pd.notna(perfect_slope):
                    data.loc[data.index[i], 'Perfect_Dynamic_Multiplier'] = 0.15 + 0.2 / (1 + 10000 * np.abs(perfect_slope))
                else:
                    data.loc[data.index[i], 'Perfect_Dynamic_Multiplier'] = 0.15
...
def main():
    ...
    output_columns = [
        ...
        'Perfect_SMA_5', 'Perfect_SMA_13', 'Perfect_SMA_13_Slope', 'Perfect_Volatility_Ratio',
